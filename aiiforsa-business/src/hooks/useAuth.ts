...
export function useLogin() {
    ...
    return useMutation({
        ...
        onSuccess: data => {
            ...
            // Route based on user role and company profile status
            if (data?.user.role === 'BUSINESS') {
                if (data.user.hasCompanyProfile) {
                    // Business user with complete profile -> dashboard
                    router.push('/dashboard');
                } else {
                    // Business user without profile -> create company profile
...
export function useRegister() {
    ...
    return useMutation({
        ...
        onSuccess: data => {
            ...
            if (data?.user.role === 'BUSINESS') {
                // Business user needs to create company profile
                router.push('/company/setup');
            } else {
                // Regular user -> home page
                router.push('/');
