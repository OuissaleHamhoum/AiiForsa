...
import { ApplicationStatus, Job, JobType } from '@/types/job.types';
...
export function ModifyForm({
    ...
    const [formData, setFormData] = useState({
        ...
        jobType: '',
        salaryRange: jobApplication.salary,
        status: jobApplication.status,
    });

    // Fetch job details to get the job type
    useEffect(() => {
        if (open && jobApplication.jobId) {
            const fetchJobDetails = async () => {
                const result = await getJobById(parseInt(jobApplication.jobId));
                if (result.success && result.data) {
                    setJobDetails(result.data);
                    setFormData(prev => ({
                        ...prev,
                        jobType: result.data!.type || '',
                    }));
                }
            };
            fetchJobDetails();
        }
    }, [open, jobApplication.jobId]);

    // Update form data when jobApplication changes
    useEffect(() => {
        if (open) {
            setFormData({
                company: jobApplication.company,
            ...
            const [day, month, year] = jobApplication.appliedDate.split('/');
            if (day && month && year) {
                setDate(
                    new Date(
                        parseInt(year),
    ...
    }, [open, jobApplication, jobDetails]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!session?.user?.id) {
            toast.error('Authentication required', {
                description:
                    'You must be logged in to update a job application',
            });
            return;
        }
        ...
        try {
            ...
            // Update the Job Application applied date if changed
            const appliedAtDate = date || new Date();
            const applicationResult = await updateJobApplication(
                jobApplication.id,
                {
            ...
            if (applicationResult.success) {
                // Update local status
                updateLocalStatus(jobApplication.id, formData.status);
    ...
    const handleDelete = async () => {
        ...
        try {
            const result = await deleteJobApplication(jobApplication.id);

            if (result.success) {
                toast.success('Job application deleted successfully!', {
                    description: `${jobApplication.title} at ${jobApplication.company}`,
                });
                onOpenChange(false);
                onSuccess?.();
            } else {
                throw new Error(
                    result.error || 'Failed to delete job application',
                );
            }
        } catch (error) {
                        ...
                        <div className="space-y-5">
                            ...
                            <div className="grid grid-cols-2 gap-3 pt-4">
                                <Button
                                    ...
                                    className="h-10 bg-red-500/10 border border-red-500/30 text-red-400 hover:bg-red-500/20 hover:border-red-500/50 hover:text-red-300 transition-all font-medium"
                            ...
                            <Button
                                type="submit"
                                className="w-full h-10 bg-[#cf6318] hover:bg-[#b55516] text-white font-semibold shadow-lg shadow-[#cf6318]/20 hover:shadow-[#cf6318]/30 transition-all"
                                disabled={loading || deleting}
                            >
