...
export async function checkServerHealth(): Promise<{
    ...
    try {
        // Respect client-visible toggles: if health check is disabled or we're mocking the API,
        // report healthy to avoid blocking UX during local development.
        if (
            process.env.NEXT_PUBLIC_DISABLE_HEALTH_CHECK === 'true' ||
            process.env.NEXT_PUBLIC_USE_MOCK_API === 'true'
        ) {
            return {
                isHealthy: true,
                message:
                    'Health check disabled in env (development convenience).',
